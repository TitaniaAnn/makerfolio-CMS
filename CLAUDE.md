...
# Run the full test suite (190 tests; ~17 skip cleanly when GD isn't loaded)
composer test
# or directly:
vendor/bin/phpunit
...
Admin UI: `/admin/settings/sample-content.php`. Drops in 5 pieces / 3 events / 2 announcements / 4 products / 3 social links with generated earthy-toned SVG placeholder images written into `public/uploads/{pottery,products,announcements}/` (the `pottery/` upload subdir name is unchanged by the table rename). To re-seed, wipe the demo rows first via `bin/reset-content.php` (or the admin Reset Content page) — that clears the `sample_content_seeded` marker.
...
- `ImageDeleteHandler.php` / `ImageCropHandler.php` / `ImageRotateHandler.php` — shared backends for the piece/shop gallery image endpoints (delete / crop / rotate), required per-endpoint (not auto-loaded by bootstrap). Delete removes the file + row, promotes the next image to primary, and syncs the parent row's `image_path` (+ optional `image_thumb`). Crop/rotate write the transformed image under a NEW filename (cache-bust), regenerate the thumb, and repoint the gallery row + parent cover. The interactive cropper UI is the dependency-free [public/admin/js/image-cropper.js](public/admin/js/image-cropper.js).
- `TemplateFileUploader.php` — validates + saves uploads for `piece_templates` (PDF/SVG/PNG/JPG/WEBP/ZIP, 50MB max).
...
- `Theme.php` — admin-configurable theme system. 6 named presets (`terra-gold`, `cool-sage`, `monochrome`, `coastal-blue`, `rosewood`, `plum-dusk`) plus optional per-role color overrides for all six palette roles (primary/accent/background/surface/text/cool — see `overridableRoles()`), font choice (display + body + eyebrow), and radius/shadow scales. `styleBlock()` returns a **nonce-stamped** `<style id="theme-overrides">:root{…}</style>` block (the nonce satisfies the strict style-src CSP — see Hardening headers) emitted by [templates/nav.php](templates/nav.php); `previewFromRequest()` decodes `?_theme_preview=<base64-json>` for the admin live-preview iframe (admin-only — anonymous visitors silently fall back).
...
**Hardening headers** — `includes/bootstrap.php` emits `Referrer-Policy: strict-origin-when-cross-origin` (kills CSRF-token-in-referer-log leak from `?csrf=` GET-style admin links), `X-Content-Type-Options: nosniff`, `X-Frame-Options: SAMEORIGIN` (the latter allows the admin theme-preview iframe but denies third-party embedding), and a **strict `Content-Security-Policy`**. `script-src` and `style-src` carry **no `'unsafe-inline'`** — every inline `<script>`, `on*=` handler, inline `<style>` block, and `style="…"` attribute was extracted to external `.js`/`.css` (under `public/js/`, `public/admin/js/`, `public/css/pages/`, `public/admin/css/pages/`). The only inline styles left are nonce-stamped via `csp_nonce()` (`Theme::styleBlock` + `public/404.php`); `public/500.php` links external CSS because it loads no bootstrap. So the policy now blocks **inline-script and inline-style injection** on top of remote-script injection, object/embed injection, form hijacking, mixed-content downgrades, `<base>` injection, and third-party framing. The admin-trusted `social_posts.embed_code` field still works — Instagram/TikTok embeds load via their allowlisted CDN `<script src>` + frame hosts. **Keep it strict**: never add an inline `<script>`/`on*=`/`<style>`/`style=` — put JS in an external file (use `data-*` + a delegated listener for handlers), CSS in a per-page file under `css/pages/` (set dynamic values via JS/CSSOM or SVG presentation attributes, or a nonce'd `<style>` only for genuinely request-time-dynamic CSS). Adding a new CDN dependency requires extending the relevant `-src` directive in `bootstrap.php`. (This strict policy replaced the original `'unsafe-inline'` one across a 4-slice refactor mirroring the SaaS's Phase 7H.)
...
- **The portfolio table is `piece`** (renamed from `pottery` in migration 024, mirroring the SaaS): `piece`, `piece_images`, `event_piece`, `piece_templates`, `piece_template_files`, FK column `piece_id`, and `announcement_links.entity_type = 'piece'`. The admin dir is `public/admin/pieces/` and the reorder kind is `'piece'`. Intentionally **still `pottery`** (do not rename): the upload subdir `public/uploads/pottery/` + `pottery_` filename prefix (renaming would orphan files on disk), the `pottery_show`/`pottery_sale` `event_type` ENUM values, the `pottery_portfolio` database name, and the `.pottery-card`/`.pottery-show` CSS classes. "Pottery" as product copy (this is the pottery self-host CMS) stays everywhere.
- **No inline scripts, handlers, or styles** — the strict CSP (see Hardening headers) forbids inline `<script>`, `on*=` attributes, inline `<style>`, and `style="…"` attributes. Put JS in an external file under `public/js/` or `public/admin/js/` (use `data-*` + a delegated listener instead of `on*=`); put CSS in a per-page file under `public/css/pages/` or `public/admin/css/pages/` (set dynamic values via JS/CSSOM or SVG presentation attributes; a nonce'd `<style>` via `csp_nonce()` is only for genuinely request-time-dynamic CSS).
...
- `/admin/settings/sample-content.php` — one-click demo-data seeder (5 pieces, 3 events, 2 announcements, 4 products, 3 social links + SVG placeholders). Idempotent via the `sample_content_seeded` settings marker. See [includes/SampleContent.php](includes/SampleContent.php).
...
Plus per-resource admin sections under `/admin/{pieces,shop,events,announcements,social,templates,orders}/`. (The portfolio admin dir is `pieces/` — renamed from `pottery/` along with the table rename; `templates/` still manages the `piece_templates` downloads feature.)

**Drag-to-reorder** is wired on the admin lists for piece, products, events, and the homepage's `page_sections`. Each row exposes a `⋮⋮` handle (`.reorder-handle`); SortableJS captures the drop and `reorder.js` POSTs the new order to `/admin/reorder.php` with the page's CSRF token (from the `<meta name="csrf-token">` tag). The endpoint validates `kind` against `ListReorder::ALLOWED` (or `PageSections::CATALOG` for page_sections), then rewrites `sort_order` in a single transaction at gaps of 10. To add reorder to a new resource: add an entry to `ListReorder::ALLOWED`, add `data-reorder-kind="…"` to the tbody/list, give each row `data-id="…"`, and include the two scripts at page bottom. The admin list pages intentionally ORDER BY pure `sort_order` (not `featured DESC, sort_order`) so the drag result is what the user sees.

**Admin list search + pagination** uses `ListQuery` + the shared [partials/list-toolbar.php](public/admin/partials/list-toolbar.php) on piece, shop, and events index pages. Per-page is 25 (capped at 100), search columns are per-page allowlisted (e.g. piece searches `title`/`description`/`technique`), and search state is bookmarkable via GET. Drag handles are conditionally rendered — disabled whenever the list is paginated or filtered (would mis-renumber a partial result).
